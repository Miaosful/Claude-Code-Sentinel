...
HOOK_NAME="cc-sentinel-hook"
HOOK_BINARY="$APP_MACOS/$HOOK_NAME"
...
if [[ ${#BUILD_FLAGS[@]} -gt 0 ]]; then
    ...
    swift build --product "$HOOK_NAME" "${BUILD_FLAGS[@]}"
else
    swift build --product "$APP_NAME"
    swift build --product "$HOOK_NAME"
fi
BUILD_BINARY="$(swift build --show-bin-path)/$APP_NAME"
BUILD_BIN_DIR="$(swift build --show-bin-path)"
BUILD_HOOK_BINARY="$BUILD_BIN_DIR/$HOOK_NAME"
BUILD_RESOURCE_BUNDLE="$BUILD_BIN_DIR/CCSentinel_CCSentinelApp.bundle"

rm -rf "$APP_BUNDLE"
mkdir -p "$APP_MACOS"
cp "$BUILD_BINARY" "$APP_BINARY"
cp "$BUILD_HOOK_BINARY" "$HOOK_BINARY"
chmod +x "$APP_BINARY"
chmod +x "$HOOK_BINARY"
